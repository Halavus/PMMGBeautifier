...
export class OrderInventory {
  ...
  private sortInventoriesByCode() {
    if (this.sortByCode) {
      const inventories = Array.from(document.querySelectorAll("div[class^='InventoryView__grid__'"));
  ...
  private addSortByCodeButton() {
    const buttonContainer = Array.from(document.querySelectorAll("div[class^='InventorySortControls__controls___'"));
